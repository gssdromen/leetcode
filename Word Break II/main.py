...
class Solution:
    ...
    #     if not s:
    #         return []
    #     in_dict = [[] for i in xrange(len(s) + 1)]
    #     in_dict[0].append(0)
    ...
    # 从后向前接续字符串
    def wordBreak(self, s, dict):
        # 到达此index是从哪个index过来的
        self.all_flags = self.is_valid(s, dict)
        self.helper(s, dict, '', len(s))
        return self.final_result

    def helper(self, s, dict_, result, index):
        # print result
        if index == 0:
            self.final_result.append(result)
        else:
            for item in self.all_flags[index]:
                temp = result
                if index == len(s):
                    result = s[item:index]
                else:
                    result = s[item:index] + ' ' + result
                self.helper(s, dict_, result, item)
                result = temp

    def is_valid(self, s, dict):
        flags = [[] for i in xrange(len(s) + 1)]
        flags[0].append(0)
